...
const columns = [
    { field: 'id', display: '#ID' },
    { field: 'name', display: 'Nama Tahun Ajaran' },
    { field: 'semester', display: 'Semester', component: (row) => row.semester % 2 ? 'Genap' : 'Ganjil' },
    { field: 'start_date', display: 'Tanggal Mulai' },
    { field: 'end_date', display: 'Tanggal Selesai' },
...
const deleteItem = async (id) => {
    alretConfirm('delete').then(async (result) => {
        if (result.isConfirmed) {
            const { ok, data, error } = await apiRequest(`academic-years/${id}`, {
                method: 'delete',
            })
...
const save = async (row) => {
    alretConfirm('save').then(async (result) => {
        if (result.isConfirmed) {
            loading.value = true
            let endpoint = `academic-years`
            if (row.id) {
                endpoint = `academic-years/${row.id}`
            }
...
<template>
    ...
    <section class="content">
        <TableServerSide
            ref="tableRef"
            title=""
            :show-add-button="checkPermission('master_data.academic_years.create')"
            :show-drawer="true"
            :columns="columns"
        ...
        >
            ...
            <template #cell-action="{ row }">
                <div class="btn-group">
                    <button
                        v-if="checkPermission('master_data.academic_years.update') && row.editable == true"
                    ...
                    <button
                        v-if="
                            checkPermission('master_data.academic_years.delete') &&
                            row.deletable == true
                        "
