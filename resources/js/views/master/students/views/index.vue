...
const columns = [
    { field: 'id', display: '#ID' },
    { field: 'nis', display: 'NIS' },
    { field: 'name', display: 'Nama' },
    { field: 'gender', display: 'Jenis Kelamin' },
...
const deleteItem = async (id) => {
    alretConfirm('delete').then(async (result) => {
        if (result.isConfirmed) {
            const { ok, data, error } = await apiRequest(`students/${id}`, {
                method: 'delete',
            })
...
<template>
    <section class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h4 class="font-serif-formal">Data Siswa</h4>
                </div>
                <div class="col-sm-6">
    ...
    <section class="content">
        <TableServerSide
            ref="tableRef"
            title=""
            :show-add-button="checkPermission('master_data.students.create')"
            :show-drawer="true"
            :columns="columns"
        ...
        >
            <template #cell-gender="{ row }">
                <span :class="`badge ${genderMap[row.gender]?.class || 'badge-secondary'}`">
                    {{ genderMap[row.gender]?.text || row.gender }}
                </span>
            </template>
            ...
            <template #cell-action="{ row }">
                <div class="btn-group">
                    <button
                        v-if="checkPermission('master_data.students.update') && row.editable == true"
                    ...
                    <button
                        v-if="
                            checkPermission('master_data.students.delete') &&
                            row.deletable == true
                        "
